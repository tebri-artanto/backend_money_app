  ...
  const predictFinances = async (userId, currentMonth) => {
    try {
      ...
      // Convert data to tensors
      const financeData = tf.tensor(historicalData.map((data) => [
      ...
      const trainData = financeData.slice(0, trainSize);
      const testData = financeData.slice(trainSize);
  
      // Create and train the linear regression model
      const model = await trainLinearRegressionModel(trainData.slice(0, -1), trainData.slice(-1));
  
      // Make predictions for the next month
      const predictedFinance = model.predict(testData.slice(-1)).dataSync();
